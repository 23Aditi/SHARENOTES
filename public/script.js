...
function getAccessToken() {
    return localStorage.getItem('sn_access');
}
function getRefreshToken() {
    return localStorage.getItem('sn_refresh');
}
function setTokens(access, refresh) {
    localStorage.setItem('sn_access', access);
...
/* Token refresh queue — prevents race conditions when multiple
...
/* Auth-aware fetch wrapper. Automatically attaches Bearer token
   and retries once on 401 after refreshing. */
...
async function checkAuth() {
    ...
    try {
        ...
        /* authFetch may have already redirected */
...
/* Refresh the access token every 13 minutes (15 min lifetime − 2 min buffer) */
...
function getFileTypeInfo(fileKey) {
...
/* ===== API Functions (all use authFetch) ===== */

async function fetchNotes() {
    state.isLoading = true;
    renderNotes();
    try {
        const params = new URLSearchParams({ page: state.currentPage, limit: state.limit, search: state.searchQuery });
...
function renderNotes() {
    ...
    if (filtered.length === 0) {
        ...
        document.getElementById('emptyTitle').textContent = isSearching
            ...
            : isFiltering ? `No ${FILTERS.find(f => f.key === state.filterType)?.label || ''} notes found` : 'No notes shared yet';
        document.getElementById('emptyDesc').textContent = isSearching
            ? 'Try different keywords or clear the search'
            : isFiltering ? 'Try selecting a different category or search instead' : 'Be the first to share your notes and help fellow students!';
    ...
    grid.innerHTML = filtered.map((note, i) => {
        ...
        return `
            <article class="note-card" style="animation-delay:${i * 0.05}s">
...
function renderFilters() {
    document.getElementById('filterBar').innerHTML = FILTERS.map(f => `
        <button class="filter-tab ${state.filterType === f.key ? 'active' : ''}" onclick="setFilter('${f.key}')">
...
function renderResultsInfo() {
    ...
    const filteredCount = state.filterType !== 'all'
        ...
        : state.notes.length;
    const filterLabel = state.filterType !== 'all' ? ` matching ${FILTERS.find(f => f.key === state.filterType)?.label || ''} filter` : '';
    el.textContent = `Showing ${filteredCount} note${filteredCount !== 1 ? 's' : ''}${filterLabel} of ${state.total} total`;
}
...
function resetUploadForm() {
    ...
    document.getElementById('uploadDept').value = '';
...
function setFilter(type) { state.filterType = type; renderFilters(); renderNotes(); renderResultsInfo(); }
...
async function handleDownload(id) {
    try { showToast('Generating download link...', 'info'); const { url } = await getDownloadUrl(id); window.open(url, '_blank'); }
...
async function handleEdit() {
    ...
    try { await updateNote(state.editingNote._id, title, desc); showToast('Note updated successfully', 'success'); closeEditModal(); fetchNotes(); }
    catch (err) { console.error('Edit error:', err); showToast(err.message || 'Failed to update note', 'error'); }
}

async function handleDelete() {
    if (!state.deletingId) return;
    try { await deleteNote(state.deletingId); showToast('Note deleted', 'success'); closeDeleteModal(); fetchNotes(); }
...
function resetToHome() {
    ...
    renderFilters(); fetchNotes(); window.scrollTo({ top: 0 });
}
...
function init() {
    ...
    document.getElementById('uploadTitle').addEventListener('input', validateUploadForm);
    document.getElementById('uploadDesc').addEventListener('input', validateUploadForm);
    document.getElementById('uploadYear').addEventListener('change', validateUploadForm);
    document.getElementById('uploadSemester').addEventListener('change', validateUploadForm);
    document.getElementById('uploadDept').addEventListener('change', validateUploadForm);
    document.getElementById('uploadSubject').addEventListener('input', validateUploadForm);
    document.getElementById('uploadUnit').addEventListener('input', validateUploadForm);
    ...
    /* Auth check — this replaces the old fetchNotes() call */
    checkAuth();
}
